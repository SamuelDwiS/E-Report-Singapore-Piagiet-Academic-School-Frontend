...
function levelStyle(level: Level) {
  if (level === "Exceeding") return { badge: "bg-success-100 text-success-700 dark:bg-success-900/30 dark:text-success-400", text: "text-success-600 dark:text-success-400", bar: "bg-success-500" };
  if (level === "Meeting")   return { badge: "bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:text-brand-400",       text: "text-brand-600 dark:text-brand-400",       bar: "bg-brand-500" };
  if (level === "Improving") return { badge: "bg-warning-100 text-warning-700 dark:bg-warning-900/30 dark:text-warning-400",   text: "text-warning-600 dark:text-warning-400",     bar: "bg-warning-500" };
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-gray-800 px-6 py-4 flex flex-wrap gap-5 items-center shadow-sm">
        ...
        {[
          { label: "Exceeding Expectations", range: "≥ 2.50", dot: "bg-success-500" },
          { label: "Meeting Expectations",   range: "2.00 – 2.49", dot: "bg-brand-500" },
          { label: "Improving",              range: "< 2.00", dot: "bg-warning-500" },
        ].map((s) => (
            ...
            <div key={slug} className="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${report.status === "Aktif" ? "bg-success-100 text-success-700 dark:bg-success-900/30 dark:text-success-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}>
                    {report.status}
                  </span>
                </div>
                ...
                <Link
                  ...
                  className="flex items-center gap-1.5 text-xs font-bold text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-800 px-3 py-1.5 rounded-xl hover:bg-brand-50 dark:hover:bg-brand-900/30 transition"
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" />
                  </svg>
                  <span className="hidden sm:inline">Lihat Raport</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-6 py-4">
                {[
                  ...
                  <div key={item.label} className="col-span-1 border-gray-100 dark:border-gray-800 sm:border-l sm:first:border-l-0 sm:pl-4 first:pl-0 sm:pt-0">
